...
function Home() {
  ...
  useEffect(() => {
    if(!localstorage) history.push('/login')
    else{
    fetch('https://instaclne.herokuapp.com/allpost', {
      headers: {
  ...
  console.log(data);
  const likePost = (id) => {
    fetch('https://instaclne.herokuapp.com/like', {
      method: 'Put',
      headers: {
  ...
  const unlikePost = (id) => {
    fetch('https://instaclne.herokuapp.com/unlike', {
      method: 'Put',
      headers: {
        'Content-Type': 'application/json',
        "authorization": localstorage.replace(/['"]+/g, '')
  ...
  const makeComment = (text, postId) => {
    fetch('https://instaclne.herokuapp.com/comment', {
      method: 'Put',
      headers: {
        'Content-Type': 'application/json',
        "authorization": localstorage.replace(/['"]+/g, '')
  ...
  const deletePost = (id) => {
    fetch(`https://instaclne.herokuapp.com/deletepost/${id}`, {
      method: 'delete',
      headers: {
        "authorization": localstorage.replace(/['"]+/g, '')
      },
  ...
  const deleteComment = (id, postId) => {
    fetch(`https://instaclne.herokuapp.com/deletecomment/${id}`, {
      method: 'delete',
      headers: {
        "authorization": localstorage.replace(/['"]+/g, ''),
        'Content-Type': 'application/json'
            ...
            <div key={index} className='home'>
              <div className='card home-card'>
                ...
                <div className='card-image' style={{ border: ' solid 1px white' }}>
                  <img alt='sorry'
                    src={item.photo} style={{ border: ' solid 1px white' }} onClick={
                        ()=>{
